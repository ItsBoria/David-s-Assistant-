...
Implementation baseline: Phase 1, initial Phase 2 app slices, the Phase 3 pure deterministic planner core, and the Phase 4 pure recurrence generator core
...
- A pure Phase 4 recurrence core: validated daily, Sunday-to-Thursday workday, multi-week selected-weekday, monthly, and custom day/week/month generation; inclusive date/count endings; stable date keys and absolute sequence numbers; and incremental cursor continuation. It does not yet write occurrence rows.
- A Phase 4 mission-occurrence persistence boundary: one recurrence rule is enforced per mission-or-meeting root, and an authenticated owner-derived PostgreSQL transaction materializes stable mission occurrences, advances/deactivates the rule cursor, and records audit evidence idempotently. No end-user recurrence form invokes it yet.

The remaining Phase 2–8 work is still planned: broader work-schedule and mission CRUD, planner orchestration and persistence, the calendar, rescheduling, transactional recurrence persistence and editing scopes, meetings and contacts UI, Telegram and reminder workers, PDF/DOCX generation, and production hardening. The foundation schema and pure cores anticipate those capabilities; they do not make them operational.
...
The first domain data-access paths now exist for Mission Inbox, Work Hours settings, My Week, and server-side recurrence generation. `/app/inbox` delegates to `src/lib/services/missions.ts` and `src/lib/repositories/missions.ts`; `/app/settings` delegates to `src/lib/services/work-hours.ts` and `src/lib/repositories/work-hours.ts`; `/app` delegates to `src/lib/services/my-week.ts`, composing existing mission and work-hours repositories through a pure read model in `src/lib/my-week/read-model.ts`. `src/lib/services/recurrence.ts` validates a bounded mission/through-date request and delegates to the authenticated `generate_mission_recurrence_occurrences` transaction, though no UI invokes it yet. These modules use the regular SSR Supabase client plus the signed-in user's JWT, so PostgreSQL/RLS or the owner-derived RPC remains the final authorization boundary.
...
| `src/lib/recurrence/` | Pure recurrence validation and civil-date occurrence generation, including stable keys, absolute sequence numbers, termination rules, and incremental cursor continuation. | Database writes, scheduling placement, React components, reminders, or series-edit transactions. |
...
| `src/lib/repositories/` | Server-only persistence adapters that map Supabase rows/RPC responses to domain-facing DTOs. The current implementation covers Mission Inbox, Work Hours, and recurrence-generation persistence. | React components, form state, or business decisions that belong in services. |
| `src/lib/services/` | Server-only business workflow functions shared by route actions/components. The current implementation covers basic selected-date mission creation/listing, weekly work-hours saving, and validated recurrence generation. | Provider SDK details, UI state, or direct browser imports. |
...
The schema is defined by three ordered foundation migrations dated `20260711` plus `20260714000300_recurrence_generation_service.sql`. UUID primary keys use `gen_random_uuid()` from `pgcrypto`; `btree_gist` supports owner-plus-time-range indexes. All instants are `timestamptz` and are UTC on the wire. A profile stores the IANA timezone used to interpret civil dates and times. Weekdays follow PostgreSQL `extract(dow)`: Sunday `0` through Saturday `6`.
...
Normalized series rule for exactly one recurring mission or meeting. Columns: `id`, `owner_id`, `frequency`, `interval_count`, optional `interval_unit`, optional `by_weekdays`, optional `day_of_month`, `starts_on`, optional `ends_on` or `maximum_occurrences`, `timezone`, nullable `generation_cursor_date`, `is_active`, timestamps. Checks enforce valid/unique weekdays, legal dates and counts, mutually exclusive end conditions, and frequency-specific shapes. A trigger validates the IANA timezone. Partial unique indexes plus a cross-table advisory-lock trigger prevent one cursor-bearing rule from being attached to multiple mission/meeting roots. `recurrence_rules_generation_idx` supports active incremental generation. The implemented mission recurrence transaction advances its cursor; future Meeting and editing-scope services will use the same one-parent invariant. RLS: Owner CRUD.
...
A concrete stateful instance of a mission, including a single non-recurring instance or one generated recurrence. Columns: `id`, `owner_id`, `mission_id`, stable `recurrence_key`, `occurrence_date`, optional `sequence_number`, status, lock flag, reschedule count, optional duration override/notes, `generated_at`, timestamps. Composite mission FK cascades unless retained history/completion records restrict removal. The mission plus recurrence key and mission plus occurrence date are unique. The implemented recurrence transaction inserts `date:YYYY-MM-DD` keys and absolute sequence numbers, excludes existing dates, bounds one call to 3,661 processed days, advances the rule cursor, deactivates exhausted rules, and appends a correlated audit summary atomically. Status writes use the same transition/completion guard as missions. `mission_occurrences_owner_date_idx` and `mission_occurrences_generation_idx` support calendar and idempotent-generation lookups. RLS: owner `SELECT/INSERT/UPDATE`; no browser physical delete.
...
| `ensure_recurrence_rule_single_parent` | Advisory-lock trigger invariant preventing a cursor-bearing recurrence rule from belonging to multiple mission roots, multiple meeting roots, or one of each. Not exposed as RPC. |
| `generate_mission_recurrence_occurrences` | Authenticated owner-derived transaction generating one mission series through a bounded local date, excluding existing dates, assigning stable keys/sequences, advancing/deactivating its rule, and appending an audit summary. |
...
The implemented pure Phase 4 recurrence core generates concrete civil dates before scheduling. Daily cadence starts at `starts_on`; workday cadence uses Sunday through Thursday; weekly cadence is anchored to the Sunday containing `starts_on`; monthly cadence preserves the requested day and skips months that do not contain it instead of silently moving the occurrence; custom day/week/month cadence can apply selected work weekdays. End dates are inclusive and maximum-occurrence endings use absolute sequence numbers. Keys use `date:YYYY-MM-DD`, matching the database's one-occurrence-per-mission/date invariant. A `{generatedThrough, occurrenceCount}` cursor continues a series without returning processed dates. The planned database service must insert with uniqueness protection and advance the rule cursor in the same owner-scoped transaction. Editing scopes will be this occurrence, this and future, or entire series; the exact series-splitting transaction is not designed yet.
...
There are no public domain HTTP routes, Edge Functions, cron endpoints, Telegram webhooks, or export endpoints. Implemented domain writes use Server Actions or authenticated server services/repositories; recurrence generation currently has a service/RPC boundary but no action or UI.
...
| `generate_mission_recurrence_occurrences(mission,through_date)` PostgreSQL function | Incrementally materialize one existing recurring mission through an inclusive civil date. | Correlation ID, created count/rows, rule/mission IDs, processed-through date, and exhaustion flag. | `security definer`; executable only by `authenticated`; derives owner solely from `auth.uid()`. Service input requires UUID plus real local date. | Locks the owner/rule graph, validates ownership and recurrence attachment, excludes existing dates, inserts unscheduled occurrences, advances/deactivates the rule, and writes `recurrence.occurrences.generated` audit evidence. Invalid ownership, one-off missions, and ranges over 3,661 days fail atomically. |
...
| --- | --- | --- |
| Unit/Vitest | Auth normalization/bounds/errors; mission creation validation; work-hours validation; My Week read-model grouping; mission status transitions and split feasibility; error contract; safe redirects; locale/direction helpers; Sunday-to-Thursday/DST/date validation; planner interval normalization/subtraction, deterministic deadline/priority/stable ranking, allowed days, buffered blockers, fixed conflicts, caps, whole and split placement, explicit failure reasons; and recurrence daily/workday/weekly/monthly/custom cadence, endings, keys, sequences, cursor continuation, inactivity, and invalid inputs. | Schedule/override-to-UTC availability orchestration, transactional rescheduling and recurrence persistence/editing scopes, reminder times, report-data preparation. |
| Database/pgTAP | 17 foundation checks plus 22 recurrence-generation checks: schema/enums/indexes/triggers/grants/RLS structure, bootstrap/isolation, lifecycle guards, cadence/date/count semantics, stable keys/sequences, cursor advancement, exhaustion/deactivation, idempotent retry, audit evidence, one-parent rule enforcement, cross-owner and one-off rejection, range bound, and anonymous denial. | Rescheduling, recurring-meeting generation, editing scopes, completion/outbox/update idempotency, retry claims, export metadata/storage authorization. |
...
| ADR-012 / 2026-07-14 | Generate recurrence as pure owner-local civil dates with stable date keys and an explicit processed-through/count cursor; skip invalid monthly days instead of clamping them. | Embedding persistence or UTC instants in recurrence math would couple series rules to scheduling. Clamping the 31st to a month end silently changes user intent, while date-only keys align with the database's one occurrence per mission/date uniqueness. | The owner-scoped mission transaction mirrors these semantics, inserts idempotently, and advances `generation_cursor_date` atomically; future meeting/editing workflows must preserve parity. Monthly rules may have no occurrence in shorter months. |
| ADR-013 / 2026-07-14 | Treat a recurrence rule and its cursor as one series root, enforce a single mission-or-meeting parent, and materialize mission occurrences in one authenticated owner-derived transaction. | Sharing one cursor across roots could make one series skip dates when another advances it. Separate inserts/cursor/audit requests could duplicate or partially record generation. A browser-computed payload would be tamperable. | Cross-table attachment is serialized and rejected; PostgreSQL independently derives cadence dates, inserts idempotently, advances/deactivates the rule, and audits in one transaction. The TypeScript pure core remains the provider-neutral preview/test contract and parity must be maintained with pgTAP fixtures. |
...
- Recurrence rule validation, deterministic civil-date generation, repository/service parsing, one-parent enforcement, and transactional mission-occurrence insertion/cursor advancement are implemented. There is no recurring-mission creation UI/action, automatic horizon worker, recurring-meeting materialization, scheduler orchestration, reminder integration, or editing-scope transaction yet.
...
- **2026-07-14 — Deterministic recurrence core:** Added validated daily, workday, selected-weekday weekly, monthly, and custom cadence generation; inclusive date/count endings; stable occurrence keys and sequences; incremental cursor continuation; invalid-input handling; and focused unit coverage. Documented civil-date, monthly-skip, and future atomic persistence semantics in ADR-012.
- **2026-07-14 — Recurrence occurrence persistence:** Enforced one recurrence rule per mission-or-meeting series and added validated service/repository contracts plus an authenticated, owner-locked PostgreSQL generator that materializes stable mission occurrences, advances/deactivates the rule cursor, rejects unsafe ranges and cross-owner calls, and records audit evidence with dedicated pgTAP coverage.
