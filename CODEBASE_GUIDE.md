...
Implementation baseline: Phase 1, Phase 2 mission-inbox and work-hours slices, plus the Phase 3 deterministic My Week plan preview
...
- A Phase 2/3 My Week slice: signed-in users can view the current Sunday-to-Thursday week with effective work hours (date overrides take precedence), selected-date inbox missions, and a read-only suggested-time preview. The preview uses the profile timezone, preferences, saved hours, calendar blockers, and deterministic planner; it does not save sessions.
...
Current Phase 2/3 note: basic selected-date mission capture, Mission Inbox reading, normal weekly work-hours settings, date-specific custom-hours/day-off overrides, and a real read-only My Week scheduling preview are operational. Accepting/persisting that preview and the rest of the later phases remain planned.
...
| `src/components/my-week/` | Read-only My Week presentation combining effective work hours, selected-date inbox missions, suggested sessions, and explicit unscheduled reasons. | Scheduling/rescheduling algorithms or direct database queries. |
...
| `src/lib/planning/` | Pure app-facing plan-preview assembly that converts effective civil hours to UTC, clips elapsed time, invokes the deterministic planner, and maps results for presentation. | Supabase access, React components, or persistence side effects. |
...
The schema is defined by three ordered foundation migrations dated `20260711` plus the `20260714000100_date_override_service.sql` transactional function migration. UUID primary keys use `gen_random_uuid()` from `pgcrypto`; `btree_gist` supports owner-plus-time-range indexes. All instants are `timestamptz` and are UTC on the wire. A profile stores the IANA timezone used to interpret civil dates and times. Weekdays follow PostgreSQL `extract(dow)`: Sunday `0` through Saturday `6`.
...
**Status: pure core and selected-date My Week preview implemented; persistence and rescheduling remain planned for Phase 3.** The core accepts work windows already resolved to UTC, subtracts buffered blocking intervals, ranks and places candidates deterministically, and returns scheduled sessions or explicit unscheduled reasons. The My Week service now reads the owner's timezone, default buffers and work cap, weekly/date-override hours, selected-date unscheduled missions, and overlapping `calendar_items`; it converts effective hours to UTC, clips elapsed time, and renders the result as a clearly labeled preview. Neither layer mutates data. A later service must create concrete one-off occurrences, transactionally recheck conflicts, save accepted sessions, and support moved-session diffs/rescheduling.
...
| --- | --- | --- |
| Unit/Vitest | Auth normalization/bounds/errors; mission creation validation; weekly/date-override validation; My Week grouping and date-override precedence; mission status transitions and split feasibility; error contract; safe redirects; locale/direction helpers; Sunday-to-Thursday/DST/date validation; planner interval normalization/subtraction, deterministic deadline/priority/stable ranking, allowed days, buffered blockers, fixed conflicts, caps, whole and split placement, explicit failure reasons, local-hours-to-UTC preview assembly, blocker application, elapsed-time clipping, and preview cap failures. | Full multi-period override availability, transactional rescheduling, recurrence, reminder times, report-data preparation. |
| Database/pgTAP | 17 foundation checks plus 10 transactional date-override checks: schema/grants/RLS, bootstrap/isolation, core lifecycle constraints, authenticated custom-hours save, atomic conversion to day off, invalid-shape rollback, and owner removal. | Rescheduling, recurrence idempotency, completion/outbox/update idempotency, retry claims, export metadata/storage authorization. |
...
- Mission, Work Hours, and Planning service/repository implementations cover selected-date unscheduled mission creation/listing, one normal work period per Sunday-to-Thursday day, one custom period or day off per specific date, owner planning preferences, calendar blocker reads, and a read-only suggested plan. Meeting writes, Scheduling persistence, Recurrence, Completion, Reminder, Notification, Contact, Telegram, Export, and Audit service implementations do not exist yet.
...
- No saved calendar placement, recurrence, meeting, contact, completion, reminder, Telegram, or export end-user workflow exists. My Week proposes times but does not automatically create occurrences or sessions. Mission editing/deletion/completion, recurrence, flexible deadlines, weekday/date-set constraints, fixed-time mission creation, multiple weekly or override periods, breaks, and splitting controls are not implemented in the UI.
- The pure deterministic planner, rank policy, interval subtraction, splitting, caps/buffers, scheduled/unscheduled result contract, owner-timezone availability resolution, Supabase planning reads, and preview UI are implemented. Transactional occurrence/session writes, accept/change controls, moved-session diffs, conflict rechecks, and automatic rescheduling are not implemented.
