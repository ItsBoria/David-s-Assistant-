...
Implementation baseline: Phase 1, Phase 2 mission-inbox and work-hours slices, plus Phase 3 deterministic planning and transactional selected-date plan acceptance
...
- Phase 2 Settings slices: signed-in users can view and save normal Sunday-to-Thursday work hours, then replace them for a specific date with one custom work period or a day off. The date-override workflow uses validated Server Actions, shared services/repositories, RLS, and a transactional PostgreSQL function.
- A Phase 2/3 My Week slice: signed-in users can view the current Sunday-to-Thursday week with effective work hours (date overrides take precedence), selected-date inbox missions, saved sessions, and a suggested-time preview. The preview uses the profile timezone, preferences, saved hours, calendar blockers, and deterministic planner; users can atomically accept the current selected-date suggestions.
- A pure Phase 3 planner core: validated UTC planning windows and blocking intervals, deterministic mission ranking, deadline/date/weekday eligibility, buffers and daily caps, fixed-time conflict reporting, whole-session placement, feasible splitting, and explicit unscheduled reasons.
- A first Phase 3 persistence boundary: the server recomputes the preview, rejects stale client snapshots, and calls an authenticated PostgreSQL transaction that rechecks ownership, mission state, work hours, date overrides, breaks, buffers, conflicts, durations, and the daily cap before creating one-off occurrences and sessions, moving missions to `planned`, and appending an audit summary.

The remaining Phase 2–8 work is still planned: broader work-schedule and mission CRUD, advanced planner persistence/rescheduling, recurrence generation, meetings and contacts UI, Telegram and reminder workers, PDF/DOCX generation, and production hardening. The foundation schema and pure planner anticipate those capabilities; they do not make them operational.

Current Phase 2/3 note: basic selected-date mission capture, Mission Inbox reading, normal weekly work-hours settings, date-specific custom-hours/day-off overrides, a real My Week scheduling preview, and atomic acceptance of whole selected-date suggestions are operational. Rescheduling, split-session persistence, and the rest of the later phases remain planned.
...
The first domain data-access paths now exist for Mission Inbox, Work Hours settings, Date Overrides, and My Week planning. `/app/inbox` delegates to the Mission service/repository; `/app/settings` delegates to Work Hours and Date Override services/repositories; `/app` delegates to `src/lib/services/my-week.ts`, composing the owner profile/preferences, mission, weekly-hours, override, and `calendar_items` blocker data through pure My Week and plan-preview models. Those modules use the regular SSR Supabase client plus the signed-in user's JWT, so PostgreSQL/RLS remains the final authorization boundary. Date-override replacement calls the security-invoker `save_date_schedule_override` PostgreSQL function. Plan acceptance calls `src/lib/services/schedule-plan.ts`, which recomputes the preview, checks the exact expected snapshot, and invokes the security-definer `apply_selected_date_plan` transaction; ownership is always derived from `auth.uid()` and all referenced rows are revalidated.
...
| `src/components/my-week/` | My Week presentation combining effective work hours, selected-date inbox missions, saved sessions, suggested sessions, explicit unscheduled reasons, and the accept-plan control. | Scheduling/rescheduling algorithms or direct database queries. |
| `src/components/settings/` | Settings presentation, weekly work-hours form, date-override manager, and settings-specific form messages. | Direct database queries, service-role access, or scheduling decisions. |
...
| `src/lib/planning/` | Pure app-facing preview assembly and exact snapshot comparison. It converts effective civil hours to UTC, rounds elapsed time to a stable five-minute planning boundary, invokes the deterministic planner, and maps results for presentation. | Supabase access, React components, or persistence side effects. |
| `src/lib/scheduling/` | Pure deterministic interval, ranking, whole-placement, splitting, cap/buffer, and result-contract logic. | Supabase, React/Next.js APIs, civil-time database queries, or session writes. |
...
| `src/lib/repositories/` | Server-only persistence adapters that map Supabase rows/RPC results to domain-facing DTOs. The current implementation covers Mission Inbox, weekly/date-specific work hours, planning profile/preferences, calendar blockers, and selected-date plan acceptance. | React components, form state, or business decisions that belong in services. |
| `src/lib/services/` | Server-only business workflow functions shared by route actions/components. The current implementation covers basic selected-date mission creation/listing, weekly work-hours saving, date-override management, My Week preview assembly, and guarded transactional plan acceptance. | Provider SDK details, UI state, or direct browser imports. |
...
The schema is defined by three ordered foundation migrations dated `20260711`, `20260714000100_date_override_service.sql`, and `20260714000200_schedule_plan_service.sql`. UUID primary keys use `gen_random_uuid()` from `pgcrypto`; `btree_gist` supports owner-plus-time-range indexes. All instants are `timestamptz` and are UTC on the wire. A profile stores the IANA timezone used to interpret civil dates and times. Weekdays follow PostgreSQL `extract(dow)`: Sunday `0` through Saturday `6`.
...
The Mission Inbox runtime path reads and writes the `missions` table for authenticated owners using RLS-protected selected-date, unscheduled mission rows. Settings reads/writes weekly schedule tables and date-override tables. Override saves call a security-invoker function that derives `owner_id` from `auth.uid()`, uses an owner/date advisory transaction lock, validates the shape, and atomically replaces the header plus work period; removal uses owner-scoped RLS deletion. My Week reads all three inputs and applies a matching date override before weekly hours. Accepting its current suggested plan calls a security-definer function that independently derives the owner, serializes calendar writes, validates every assignment, and atomically creates occurrence/session rows plus history and audit evidence. Other service names in the catalog below still identify planned ownership boundaries, not existing executable classes.
...
One override header per owner/date. Columns: `id`, `owner_id`, `override_date`, `override_kind` (`custom_hours` or `day_off`), optional `reason`, timestamps. `(owner_id, override_date)` is unique; reason length is bounded. A trigger prevents a `day_off` row from retaining periods. The implemented Date Override service reads, saves, and removes it; My Week reads it, while Scheduling and Audit integration remain planned. RLS: Owner CRUD. Indexes: PK and owner/date unique.

#### `date_schedule_override_periods`

Work/break periods for a custom-hours override. Columns: `id`, `owner_id`, `override_id`, `period_kind`, `starts_at`, `ends_at`, timestamps. Composite FK cascades and preserves ownership; time order and exact uniqueness are checked. Triggers lock/verify the parent is `custom_hours` and reject same-kind overlap. `date_schedule_override_periods_owner_idx` supports owner/override scans. The implemented Date Override service stores one `work` period; multiple work periods and breaks remain planned. RLS: Owner CRUD.
...
| `apply_selected_date_plan` | Authenticated security-definer transaction that derives the owner from `auth.uid()`, locks that owner's calendar, rechecks selected-date assignments, creates one-off occurrences/sessions, advances missions to `planned`, and appends one correlated audit summary. |
...
**Status: pure core, selected-date My Week preview, and first transactional acceptance path implemented; rescheduling remains planned for Phase 3.** The core accepts work windows already resolved to UTC, subtracts buffered blocking intervals, ranks and places candidates deterministically, and returns scheduled sessions or explicit unscheduled reasons. The My Week service reads the owner's timezone, default buffers and work cap, weekly/date-override hours, selected-date unscheduled missions, and overlapping `calendar_items`; it converts effective hours to UTC, clips elapsed time to the next stable five-minute boundary, and renders the result. Saving never trusts client assignments: the server recomputes the preview and requires an exact snapshot match, then PostgreSQL independently rechecks ownership, mission state, hours, overrides, breaks, duration, buffered conflicts, pairwise gaps, and daily limits before writing one-off occurrences and sessions atomically. Moved-session diffs, automatic rescheduling, and split-session acceptance remain planned.
...
- `/app` is a read-only effective-hours/mission week, `/app/inbox` is the selected-date Mission Inbox, `/app/settings` manages weekly and date-specific work hours, and `/app/people` remains an honest protected empty state.
...
There is no schedulable calendar grid/day agenda, mission editor, meeting/contact editor, broader scheduling-preferences editor, Telegram linking panel, or export history UI yet. Later calendar work must use mobile day-list/agenda alternatives when a week grid is not usable.

### Components and forms

`AuthShell` supplies the responsive brand/auth layout. Auth, Mission, Work Hours, and Date Override forms use React Hook Form plus shared Zod schemas, submit to Server Actions, disable while pending, show accessible progress, and render field/form errors. `DateOverrideManager` switches between day-off and custom-hours fields, lists persisted overrides, and requires explicit confirmation before removal. `AppShell`, `ShellHeader`, and desktop/mobile navigation compose protected pages. `FoundationView` explicitly states that later services are not connected. Local UI primitives follow shadcn aliases and use `class-variance-authority`/`cn` for variants.
...
There are no public domain HTTP API routes, Edge Functions, cron endpoints, Telegram webhooks, or export endpoints yet. Implemented domain writes use authenticated Server Actions -> shared services -> repositories/PostgreSQL functions.
...
| `saveDateOverrideAction(input)` | Save/replace `{overrideDate,kind,reason,startsAt,endsAt}`. Day off ignores draft times; custom hours require start before end. | `{ok,message}`. | Signed-in owner; shared `saveDateOverrideSchema` validates client and server input. The PostgreSQL function derives ownership again from `auth.uid()`. | Calls `save_date_schedule_override`; atomically replaces one owner/date header and zero or one work period; revalidates Settings and My Week. |
| `removeDateOverrideAction({overrideDate})` | Remove one date-specific override so weekly hours apply again. | `{ok,message}`. | Signed-in owner; valid civil date; repository filters by JWT owner and RLS enforces isolation. | Deletes the override header and cascades periods; revalidates Settings and My Week. Missing/stale records return a safe generic error. |
| `save_date_schedule_override(date,kind,reason,start,end)` PostgreSQL function | Transactional write boundary used by Settings. | Saved override UUID or SQL error. | `security invoker`; executable by `authenticated`/`service_role`, revoked from `anon`/`PUBLIC`; owner comes only from `auth.uid()`. | Uses an owner/date advisory lock, validates shape/length, safely converts custom-hours ↔ day-off, and leaves no partial header/period state on failure. |
| `acceptPlanAction(expectedPlan)` | Accept all whole selected-date sessions currently suggested in My Week. The submitted snapshot contains only mission/session identities and instants and is treated as an optimistic concurrency token. | `{ok,message}`; success refreshes My Week and Mission Inbox. | Signed-in owner; strict array/UUID/explicit-offset validation, end-after-start, no duplicate missions. The service recomputes the authoritative preview and requires exact ordered equality. | Calls `apply_selected_date_plan`; stale/empty plans fail safely, and successful writes revalidate `/app` and `/app/inbox`. |
| `apply_selected_date_plan(assignments)` PostgreSQL function | Atomic selected-date plan persistence boundary. | Correlation UUID plus created occurrence/session identities and instants, or a SQL error that rolls back all assignments. | `security definer`; executable only by `authenticated`/`service_role`; owner comes only from `auth.uid()`. Mission roots and every related row are owner-checked. | Uses the same owner advisory lock as calendar overlap triggers; rechecks mission state, effective hours/overrides/breaks, durations, buffers, conflicts, gaps, and daily cap; creates one-off occurrences/sessions, changes missions to `planned`, lets status triggers write history, and appends `schedule.plan.accepted` to `audit_log`. |
...
| --- | --- | --- |
| Unit/Vitest | Auth normalization/bounds/errors; mission creation validation; weekly/date-override validation; My Week grouping and date-override precedence; mission status transitions and split feasibility; error contract; safe redirects; locale/direction helpers; Sunday-to-Thursday/DST/date validation; planner interval normalization/subtraction, deterministic deadline/priority/stable ranking, allowed days, buffered blockers, fixed conflicts, caps, whole and split placement, explicit failure reasons, local-hours-to-UTC preview assembly, blocker application, stable five-minute elapsed-time clipping, preview cap failures, and exact acceptance-snapshot comparison. | Full multi-period override availability, transactional rescheduling, recurrence, reminder times, report-data preparation. |
| Database/pgTAP | 17 foundation checks, 10 transactional date-override checks, and 19 selected-date acceptance checks: schema/grants/RLS, bootstrap/isolation, core lifecycle constraints, override atomicity, owner removal, plan ownership/state/hours/day-off/duration/conflict/gap/cap enforcement, occurrence/session/history/audit writes, anonymous denial, and rollback without partial rows. | Rescheduling, recurrence idempotency, completion/outbox/update idempotency, retry claims, export metadata/storage authorization. |
...
| ADR-011 / 2026-07-14 | Save a date override through one security-invoker PostgreSQL function instead of separate browser-visible header/period mutations. | Converting custom hours to a day off requires deleting periods before updating the header trigger; multiple HTTP statements could leave a partial valid-but-wrong state. A service-role API was unnecessary because ordinary owner CRUD already has RLS. | The function derives the owner from `auth.uid()`, serializes concurrent owner/date writes, remains subject to RLS, and must evolve when multi-period override editing is added. |
| ADR-012 / 2026-07-14 | Keep planning as a pure deterministic function over UTC-resolved half-open windows, with an explicit rank tuple and machine-readable unscheduled reasons. | Mixing database reads/writes, civil-time conversion, and placement logic would make previews hard to reproduce and failures hard to explain. Weighted or AI scoring would hide ordering changes. | Callers must resolve owner-local schedules before planning and persist only after preview/transactional conflict recheck; identical validated inputs produce identical output. |
| ADR-013 / 2026-07-14 | Accept a suggested plan through an exact recompute-and-compare handshake followed by one owner-scoped security-definer PostgreSQL transaction. | Trusting browser-provided assignments would permit tampering; separate occurrence/session/status/audit requests could partially succeed or race calendar changes. | The expected client snapshot is only a stale-view guard. The server recomputes it, and PostgreSQL derives ownership and rechecks every invariant while holding the same owner advisory lock as overlap triggers. Current acceptance is deliberately limited to whole, one-off selected-date missions. |

## 17. Known limitations

- Phase 1 is implemented plus narrow Mission Inbox, weekly/date-specific Work Hours settings, and My Week read-model slices. People and Meetings still shows an honest empty state.
- Mission, Work Hours, and Planning service/repository implementations cover selected-date unscheduled mission creation/listing, one normal work period per Sunday-to-Thursday day, one custom period or day off per specific date, owner planning preferences, calendar blocker reads, suggested planning, and atomic acceptance of whole selected-date suggestions. Meeting writes, rescheduling, Recurrence, Completion, Reminder, Notification, Contact, Telegram, and Export service implementations do not exist yet.
...
- Saved calendar placement exists only for manually accepted, whole, one-off selected-date suggestions. Recurrence, meeting, contact, completion, reminder, Telegram, and export end-user workflows do not exist. Mission completion, recurrence, flexible deadlines, weekday/date-set constraints, fixed-time mission creation, multiple weekly or override periods, breaks, and splitting controls are not implemented in the UI.
- The pure deterministic planner, rank policy, interval subtraction, splitting, caps/buffers, scheduled/unscheduled result contract, owner-timezone availability resolution, Supabase planning reads, preview UI, stale-preview guard, conflict rechecks, and initial transactional occurrence/session writes are implemented. Plan editing, moved-session diffs, split persistence, and automatic rescheduling are not implemented.
...
- Settings lists at most the next 50 current/future date overrides and currently supports only one `work` period per date; historical browsing, multiple periods, break containment, and unavailable-time editing remain future work.
...
- **2026-07-14 — Date-specific work hours:** Added transactional custom-hours/day-off saves, owner-scoped removal, Settings management UI, My Week override precedence and reasons, a new ordered migration, validation/read-model unit coverage, and dedicated pgTAP service checks.
...
- **2026-07-14 — Deterministic planner core:** Added the pure scheduling input/result contract, validated UTC half-open interval normalization and subtraction, explicit deadline/priority/stable ordering, date and weekday eligibility, buffered blockers, daily mission caps, fixed-time validation, whole and split placement, machine-readable unscheduled reasons, and focused unit coverage. Documented the reproducibility boundary and remaining orchestration/persistence work in ADR-012.
- **2026-07-14 — My Week plan preview:** Connected the planner to owner timezone/preferences, effective weekly/date-override hours, selected-date inbox missions, and RLS-filtered calendar blockers; clipped past time, rendered suggested sessions and actionable unscheduled reasons, and kept the workflow explicitly read-only pending transactional persistence.
- **2026-07-14 — Transactional plan acceptance:** Added a strict optimistic preview handshake, stable five-minute planning boundary, guarded Server Action/service/repository path, saved-session UI, and an authenticated owner-locked PostgreSQL transaction that rechecks planning invariants, writes one-off occurrences/sessions, advances mission status, records history, and appends a correlated audit summary with rollback-focused pgTAP coverage.
